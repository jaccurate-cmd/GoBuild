...
| Extrude edge(s) | ✅ Complete | `EdgeExtrudeOperation.apply`; boundary and interior edges; new quad face `[vb, va, na, nb]` — walks the shared edge opposite to the adjacent face so both stay consistently oriented; panel button + `Shift+drag` in Edge mode; 16 unit tests; undo/redo via `apply_operation` |
...
| Cut (knife) | ✅ Complete | `CutOperation.apply(mesh, face_index, edge_a, t_a, edge_b, t_b)`; splits one face along a line drawn between two points on its boundary; mid-edge cut points are threaded into every adjacent face so no T-junction is left behind; points snap onto existing corners; carries material, smooth group, and interpolated UVs onto both halves; `CutOperation.can_apply` shares the same validation so the preview cannot disagree with the commit. Driven interactively by `GoBuildCutController` (click edge point → click edge point, Ctrl quarter-snap, Esc to back out) with the rubber-band preview drawn by `CutOverlay`; panel button in Edge section + `K` shortcut; one undo step per cut; 19 operation + 28 controller unit tests |
